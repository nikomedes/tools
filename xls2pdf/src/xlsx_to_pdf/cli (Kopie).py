...
def find_last_used_column(ws) -> int:
    """
    Ermittelt die letzte fachlich benutzte Spalte.
    Nutzt echte Zellinhalte + Merge-Bereiche.
    Ignoriert das irreführende ws.max_column (hier XFD).
    """
    last_col = 1

    # echte belegte Zellen
    for row, col in ws._cells.keys():
        value = ws.cell(row=row, column=col).value
        if value not in (None, ""):
            last_col = max(last_col, col)

    # verbundene Bereiche mit berücksichtigen
    for merged_range in ws.merged_cells.ranges:
...
def build_pdf(output_file: Path, header_rows: list[list[str]], data_row: list[str]) -> None:
...
def split_xlsx(input_xlsx: Path, output_dir: Path, sheet_name: str | None = None) -> None:
    ...
    for r in range(3, last_row + 1):
        ...
        build_pdf(output_file, header, row)
        created += 1
